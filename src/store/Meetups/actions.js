...
export default {
    ...
    async createMeetups(context, payload) {
        context.dispatch('loading', true)
        axios.post('http://localhost:1337/meetups', payload).then(response => {
            context.dispatch('getMeetups');
            context.dispatch('loading', false);
            context.dispatch('toast', true);
            context.dispatch('toastTitle', 'success');
            context.dispatch('toastDesc', 'Data has been fetched successfully');
            ...
            context.dispatch('loading', false);
            context.dispatch('modal', true);
            context.dispatch('modalTitle', 'Failed');
            context.dispatch('modalDesc', sd);

        });

    }, async updateMeetups(context, payload) {
        console.log(payload)
        axios.put(`http://localhost:1337/meetups/${payload.id}`, payload).then(() => {
            context.dispatch('getMeetups');
            context.dispatch('loading', false);
            context.dispatch('toast', true);
            context.dispatch('toastTitle', 'success');
            context.dispatch('toastDesc', 'Meetup has been Updated successfully');
          ...
          /*  let sd = ''
            ...
            context.dispatch('modal', true);
            context.dispatch('modalTitle', 'Failed');
            context.dispatch('modalDesc',  error.message);

        })
    },
    async deleteMeetup(context, payload) {
        console.log(payload)
